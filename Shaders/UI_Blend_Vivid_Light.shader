Shader "UI/Blend/Vivid Light"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 VividLight (fixed4 blend, fixed4 target)
			{ 
				fixed4 result = saturate(target > .5 ? blend / (1.0 - (target - .5) * 2.0) : 1.0 - (1.0 - blend) / (target * 2.0));
				result.a = target.a;
				return result;
			} 
